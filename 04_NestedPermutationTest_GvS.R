...
##
##   Animal = randomization block ("unit"). Region and hemisphere are not
##   consistently crossed within animal, so hemisphere is collapsed and the
##   contrast is ACCs vs ACCg within animal, BMI sessions only.
##   Because the number of sessions at each site varies between animals, the
##   number of comparison-level sessions is kept constant during the shuffles.
##   Exact space for the current design: 69,300 arrangements.
##
...
## ---- CONFIG ----
## Region labels are exchangeable within animal.
...
analyze_dataset <- function(data_path) {
  ...
  missing_cols <- setdiff(c(subject_col, treatment_col, measure_cols), names(df))
  ...
  ## Unit is the animal; hemisphere is collapsed.
  df$unit <- factor(df[[subject_col]])
  ...
  if (any(unit_counts == 0L))
    stop("Every animal must contribute sessions at both sites.")
  ...
  ## Unit and subject are the same here, so the subject-level mean is the
  ## identity.
...
invisible(lapply(names(datasets), function(tag) {
  cat("\n==== ", tag, " (", datasets[[tag]], ") ====\n", sep = "")
